...
import br.usp.larc.sembei.capacitysharing.crypto.keyagreement.SMQVTests;
...
public class MainActivity extends Activity {
	...
	MSSCryptoProvider mss;
		...
		mss.keyGen();
		Log.i("CASH", "mss.getPkey(): " + mss.getPkey());
		Log.i("CASH", "mss.getSkey() - before: " + mss.getSkey());
		String signature = mss.sign(msg);
		Log.i("CASH", "signed_msg: " + signature);
		Log.i("CASH", "verify: " + mss.verify(msg, signature, mss.getPkey()));
		Log.i("CASH", "mss.getSkey() - after: " + mss.getSkey());

		/**
		 * HMAC
		 */
		String key = mss.symmetric_keyGen();
		Log.i("CASH", "session_key: " + key);
		String hmac = mss.get_hmac(msg, key);
		Log.i("CASH", "mss.verify_hmac: " + mss.verify_hmac(msg, key, hmac));
		...
		Log.i("CASH", "mss.symmetric_encrypt: " + sym_enc_msg);
		String sym_dec_msg = mss.symmetric_decrypt(sym_enc_msg, iv, key);
		Log.i("CASH", "mss.symmetric_decrypt: " + sym_dec_msg);
		...
		Log.i("CASH", "mss.asymmetric_encrypt: " + asym_enc_msg);
		String asym_dec_msg = mss.asymmetric_decrypt(asym_enc_msg, skey);
		Log.i("CASH", "mss.asymmetric_decrypt: " + asym_dec_msg);
		
		/**
		 * Hash
		 */
		Log.i("CASH", "mss.get_hash: " + mss.get_hash(msg));
		...
		String certificateFieldsStr = "";
		for(int i = 0; i < certificateFields.length; i++)
			certificateFieldsStr += certificateFields[i].toString() + '\n';
		Log.i("CASH", "mss.readCert: " + certificateFieldsStr);
		
		 /*
		  * SMQV
		  */
		Log.i("CASH", "SWQV: " + SMQVTests.run());
